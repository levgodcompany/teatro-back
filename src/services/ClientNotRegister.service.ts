...
import { AppointmentModel, ClientNotRegisterModel, RoomModel } from "../models/schema/ISchema.schema";

class ClientNotRegisterService {
  ...
  async createClient(
    ...
    try {
        const client = await ClientNotRegisterModel.findOne({ email: email });
        if(!client){
            const newClient = await ClientNotRegisterModel.create({
              name,
              email,
              phone,
            });
  ...
  async getAllClients() {
    try {
      const clients = await ClientNotRegisterModel.find();
      return clients;
    } catch (error) {
  ...
  async getClientById(clientId: string) {
    try {
      const client = await ClientNotRegisterModel.findById(clientId);
      return client;
    } catch (error) {
  ...
  async updateClient(clientId: string, updatedData: Partial<IClientNotRegister>) {
    try {
      const client = await ClientNotRegisterModel.findByIdAndUpdate(
        clientId,
        updatedData,
        { new: true }
      );
  ...
  async deleteClient(clientId: string): Promise<void> {
    try {
      await ClientNotRegisterModel.findByIdAndDelete(clientId);
    } catch (error) {
...
export default new ClientNotRegisterService()